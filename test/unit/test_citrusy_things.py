...
def test_valid_citrus_address():
    from citruslib import is_valid_citrus_address

    main = valid_citrus_address()
    test = valid_citrus_address('testnet')

    assert is_valid_citrus_address(main) is True
    assert is_valid_citrus_address(main, 'mainnet') is True
    assert is_valid_citrus_address(main, 'testnet') is False

    assert is_valid_citrus_address(test) is False
    assert is_valid_citrus_address(test, 'mainnet') is False
    assert is_valid_citrus_address(test, 'testnet') is True


def test_invalid_citrus_address():
    from citruslib import is_valid_citrus_address

    main = invalid_citrus_address()
    test = invalid_citrus_address('testnet')

    assert is_valid_citrus_address(main) is False
    assert is_valid_citrus_address(main, 'mainnet') is False
    assert is_valid_citrus_address(main, 'testnet') is False

    assert is_valid_citrus_address(test) is False
    assert is_valid_citrus_address(test, 'mainnet') is False
    assert is_valid_citrus_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from citruslib import elect_mn
...
def test_parse_masternode_status_vin():
    from citruslib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % citruslib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(citruslib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(citruslib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(citruslib.blocks_to_seconds(16616)) == 2612035
