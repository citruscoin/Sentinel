...
class CitrusDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some citrusd methods
    ...
    @classmethod
    def from_citrus_conf(self, citrus_dot_conf):
        from citrus_config import CitrusConfig
        config_text = CitrusConfig.slurp_config_file(citrus_dot_conf)
        creds = CitrusConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = citruslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = citruslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = citruslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_citrusd_proto_version_with_sentinel_ping)
