...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, citrusd):
        golist = citrusd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_citrusd(citrusd, item)
    ...
    @classmethod
    def import_gobject_from_citrusd(self, citrusd, rec):
        import decimal
        import citruslib
        import inflection
        ...
        # shim/citrusd conversion
        object_hex = citruslib.SHIM_deserialise_from_citrusd(object_hex)
        objects = citruslib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from citrusd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from citrusd, with every run
        ...
        # get/create, then sync payment amounts, etc. from citrusd - Citrusd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(citrusd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from citrusd! %s" % e)
            govobj.vote_delete(citrusd)
            return (govobj, None)
    ...
    def vote_delete(self, citrusd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(citrusd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, citrusd, signal, outcome):
        import citruslib
        ...
        output = citrusd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = citruslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(citrusd, signal)

    def sync_network_vote(self, citrusd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = citrusd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 citrus addr, non-multisig
            if not citruslib.is_valid_citrus_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Citrus address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (citruslib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CitrusDrive, etc.)
        return False
    ...
    def prepare(self, citrusd):
        try:
            object_hash = citrusd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not citruslib.is_valid_citrus_address(addr, config.network):
    ...
    def hash(self):
        import citruslib
        return citruslib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, citrusd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - citrusd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, citrusd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - citrusd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, citrusd):
        now = int(time.time())
        return (self.created_at < (now - citrusd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, citrusd):
        if self.is_expired(citrusd):
            return False

        return True

    def is_deletable(self, citrusd):
        if self.is_expired(citrusd):
            return True
