...
def has_citrus_conf():
    ...
    valid_citrus_conf = False

    # ensure citrus_conf exists & readable
    #
    # if not, print a message stating that Citrus Core must be installed and
    # configured, including JSONRPC access in citrus.conf
    try:
        f = io.open(config.citrus_conf)
        valid_citrus_conf = True
    except IOError as e:
        print(e)

    return valid_citrus_conf
...
def main():
    ...
    if not has_citrus_conf():
        print("CitrusCore must be installed and configured, including JSONRPC access in citrus.conf")
        sys.exit(1)
