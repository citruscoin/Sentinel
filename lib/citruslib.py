...
def is_valid_citrus_address(address, network='mainnet'):
    ...
    citrus_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Citrus address
        return False

    if (address_version != citrus_version):
        return False
...
# shims 'til we can fix the citrusd side
def SHIM_serialise_for_citrusd(sentinel_hex):
    ...
    # superblock => "trigger" in citrusd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # citrusd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    citrusd_hex = serialise(obj)
    return citrusd_hex


# shims 'til we can fix the citrusd side
def SHIM_deserialise_from_citrusd(citrusd_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(citrusd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('citrus.conf')
